...
export default async function main() {
  ...
  if (diff.length > 8000) {
    console.log(chalk.white('The diff is too large to write a commit message'));
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(chalk.bold('Commit message: ') + commitBotMessage + '\n');
    ...
    if (confirmationMessage.useCommitMessage === 'n') {
      console.log(chalk.white('Commit message has not been committed.'));
      process.exit(1);
    }
...
async function generateCommitMessage(prompt) {
  ...
  const commitBot = json.choices[0].text;

  return commitBot.replace(/(\r\n|\n|\r)/gm, '');
}
